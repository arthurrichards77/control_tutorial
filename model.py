...
def sys_id_cost(p):
    A2 = A + np.array([[0,0],[p[0],p[1]]])
    B2 = B + np.array([[p[2]],[p[3]]])
...
C = np.array([[1.0,0]])
D = np.array([0.0])
(t_mdl,y_mdl) = scipy.signal.step((A3,B3,C,D),T=t_data)
plt.figure()
plt.plot(t_mdl,u_ss*y_mdl,'k')
plt.plot(t_data,y_data) # compare with real data
plt.show()
...
lag_disc = lag_comp.to_ss().to_discrete(dt=0.5)
print('comp_a =',lag_disc.A[0,0])
print('comp_b =',lag_disc.B[0,0])
print('comp_c =',lag_disc.C[0,0])
print('comp_d =',lag_disc.D[0,0])